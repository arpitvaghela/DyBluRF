...
def evaluation():
    ...
    print(f"num = {num}")
    os.makedirs(os.path.join(basedir, expname, 'video'), exist_ok=True)
    # os.makedirs(os.path.join(basedir, expname, 'test'), exist_ok=True)
    
    save_deblur_path = os.path.join(basedir, expname, 'video')
    ...
    with torch.no_grad():
        ...
        for i in range(0, int(num_img)):
            ...
            if i in i_train:
                ...
                for pose_idx in [0, 3, 6]:
                    i_pose = convert3x4_4x4(torch.Tensor(spline_poses[pose_idx]))
                    ...
                    filename = os.path.join(save_deblur_path, f'rgb_{idx:05d}.jpg')
